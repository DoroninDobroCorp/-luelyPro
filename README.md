...
Интеллектуальный голосовой ассистент с AI для помощи на устных экзаменах. Распознает речь экзаменатора, генерирует тезисы-подсказки через Gemini и дает развернутые ответы.

## Архитектура

Упрощенная модульная архитектура с асинхронной обработкой:

```
main.py → live_recognizer.py ─┬─→ asr_transcriber.py (faster-whisper)
                               ├─→ thesis_generator.py (Gemini Flash)
                               ├─→ llm_answer.py (Gemini Flash)
                               ├─→ tts_silero.py (Silero TTS v4)
                               ├─→ vad_silero.py (Silero VAD)
                               └─→ config.py

Логика:
  МОЙ ГОЛОС → фильтруем (только логирование)
  ЧУЖОЙ ГОЛОС → ASR → генерация 3 тезисов → озвучка → LLM ответ → озвучка
  ВСЁ АСИНХРОННО: TTS озвучивает, микрофон продолжает слушать!
```
...
### live_recognizer.py (1858 строк)
Ядро системы: распознавание голоса, диаризация (свой/чужой), ASR, генерация тезисов, LLM ответы, TTS.

**Основные классы:**
- `VoiceProfile` - голосовой профиль пользователя (ECAPA-TDNN эмбеддинги)
- `LiveVoiceVerifier` - распознавание своего/чужого голоса в реальном времени, генерация тезисов через Gemini
